...
- 设置/新建任务/待办:视图级头部(标题+关闭)+ 居中内容列(max-w-2xl / max-w-xl);
  Esc 离开;点任何导航即切走(视图切换永远响应)。
  待办**不是空间**(2026-08-11 用户定案:待办没有自己的侧栏内容,做成空间
  就得往侧栏塞重复信息):rail 导航组末尾的开关(设置同款 aria-pressed),
  点开只换主区,侧栏保持原空间列表。渲染优先级:设置 > 新建 > 待办 > 云端
  任务 > 会话——「派发成任务」开新建视图时待办留在其下,取消创建即回来。
- 云端任务视图与会话视图同构。
