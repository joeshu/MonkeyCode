...
列表(flex-1,唯一滚动区,纵滚横截):menu menu-sm + details 折叠
  (本地任务空间列表顶部是「待办」**组**,§4:项目组同款区块标签 +
   hover「+」开行内添加;行 = **纯文字安静行**(与会话行同款,行首无
   勾选件——checkbox-xs 与 14px 圆圈钮两版都试过,2026-08-12 用户定案
   「不需要前面的小圆点」,完成走右键首项「标记完成」)+ 行尾要紧态
   状态点(派发去向回查会话表);「已完成」为组内小节折叠
   (mc.todoDoneOpen,默认收起);组折叠注册 mc.collapsedGroups 哨兵
   key \0todos。空态也保留——没有会话 ≠ 没有要记的事。未完成待办数
   进概览统计行(「N 待办」缀在任务数后,仅 >0 时出现,用户定案
   2026-08-12)。图片:行右键「添加图片」走系统选图/输入态粘贴截图
   (添加行随 Enter 挂上、编辑行立即挂);有图行行尾 13px 图片角标,
   点开 Lightbox 竖排查看 + × 移除;派发时图片预填进新建视图照旧)
